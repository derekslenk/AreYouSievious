...
| `AYS_SESSION_IDLE_TIMEOUT` | `1800` (30 min) | Seconds of inactivity after which a session stops being accepted. Its credentials are freed by the next request that touches the store — a session nobody returns to is refused immediately but stays resident until then, since there is no background sweeper. Also the `max_age` on the session and CSRF cookies, so the browser hint and the server's enforcement cannot drift apart. |
