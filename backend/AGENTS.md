...
| `tests/` | 18 pytest files, mostly regression locks tied to a bead id in the module docstring |
...
- `cd backend && python -m pytest tests/ -v` — 18 files. Install with `pip install -r requirements.txt -r requirements-dev.txt`.
