...
2. Backend validates against IMAP (port 993) using `ssl.create_default_context()` — the full system CA chain plus hostname are verified. The opt-out env var `AYS_IMAP_INSECURE=1` exists for self-signed test setups only; it disables verification and logs a warning when `build_tls_context()` builds that configuration's context (do NOT set in production — CWE-295).
